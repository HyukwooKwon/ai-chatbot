...
COMPANY_AI_MODELS = {
    name: os.getenv(f"AI_MODEL_{name}", "gpt-3.5-turbo") 
    for name in COMPANY_NAMES
}

COMPANY_PROMPTS = json.loads(os.getenv('COMPANY_PROMPTS', '{}'))

API_KEYS = {
    model: os.getenv(f"OPENAI_API_KEY_{model}") 
    for model in set(COMPANY_AI_MODELS.values())
}

ALLOWED_COMPANIES = set(COMPANY_NAMES)

def get_company_prompt(company_name):
    return COMPANY_PROMPTS.get(
        company_name, 
        "기본 상담사로서 친절하고 정확하게 응답하세요."
    )
...
def get_company_settings(company_name):
    ...
    openai_api_key = os.getenv(f"OPENAI_API_KEY_{ai_model}")
